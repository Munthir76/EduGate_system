...
import { 
  LogOut, GraduationCap, Calendar, BookOpen, FileText,
  Clock, CheckCircle, XCircle, AlertCircle, Globe, Bell,
  User, Star, Home, BarChart3, BookMarked, Award, TrendingUp
} from 'lucide-react';
...
const StudentDashboard = ({ user, onLogout }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [schedulesRes, gradesRes, assignmentsRes, attendanceRes, feesRes] = await Promise.all([
        axios.get('/schedules'),
        axios.get(`/grades?student_id=${user.id}`),
        axios.get('/assignments'),
        axios.get(`/attendance?student_id=${user.id}`),
        axios.get(`/fees?student_id=${user.id}`)
      ]);
      
      setSchedules(schedulesRes.data.filter(s => s.class_grade === user.class_name));
      setGrades(gradesRes.data);
      setAssignments(assignmentsRes.data.filter(a => a.class_name === user.class_name));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50" dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
      {/* Header */}
      <div className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-gradient-to-br from-purple-600 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg overflow-hidden">
                {user.profile_image ? (
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">{t('student')} - {user.class_name}</p>
              </div>
            </div>
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-blue-700">الواجبات</p>
                        <h3 className="text-3xl font-bold text-blue-900">{assignments.length}</h3>
                      </div>
                      <BookMarked className="w-12 h-12 text-blue-600 opacity-50" />
                    </div>
                  </CardContent>
                </Card>
              ...
              <Card>
                ...
                <CardContent>
                  {(() => {
                    ...
                    const dayName = daysOfWeek[today] || daysOfWeek[0];
                    const todaySchedule = schedules.filter(s => s.day_of_week === dayName);
                    
                    if (todaySchedule.length === 0) {
                      return <p className="text-gray-500 text-center py-4">لا توجد حصص اليوم</p>;
                    }
                          ...
                          <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-500">{schedule.teacher_name || 'المعلم'}</p>
                              </div>
                            </div>
                            <Badge variant="outline" className="text-lg px-4 py-2">{schedule.time_slot}</Badge>
                          </div>
                        ))}
                      </div>
                    );
              ...
              <Card>
                ...
                <CardContent>
                  {grades.length > 0 ? (
                    ...
                    <p className="text-gray-500 text-center py-4">لا توجد درجات مسجلة</p>
                  )}
                </CardContent>
              </Card>
            </div>
                        ...
                        {daySchedules.length > 0 ? (
                          daySchedules.map((schedule, idx) => (
                            <div key={idx} className="p-3 bg-gray-50 rounded-lg text-center">
                              ...
                              <p className="text-xs text-gray-500">{schedule.time_slot}</p>
                            </div>
                          ))
                        ) : (
                          <p className="text-gray-400 text-center text-sm py-4">لا توجد حصص</p>
                        )}
                      </CardContent>
                    </Card>
                  );
          ...
          {activeTab === 'grades' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold text-gray-900">درجاتي</h2>
                    ...
                    <tbody className="divide-y">
                      {grades.map((grade, idx) => (
                        <tr key={idx} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-sm text-gray-500">{grade.date}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <Card key={idx} className={`hover:shadow-lg transition-shadow ${isOverdue ? 'border-red-200' : ''}`}>
                      <CardContent className="p-6">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {isOverdue && (
                            <Badge className="bg-red-500">منتهي</Badge>
                          )}
                        </div>
                      </CardContent>
                    </Card>
          ...
          {activeTab === 'fees' && (
            <div className="space-y-6">
              <h2 className="text-3xl font-bold text-gray-900">الرسوم الدراسية</h2>
                        ...
                        <tr key={idx} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-center">
                            {fee.paid_amount >= fee.amount ? (
                              <Badge className="bg-green-500">مدفوع بالكامل</Badge>
                            ) : fee.paid_amount > 0 ? (
                              <Badge className="bg-yellow-500">مدفوع جزئياً</Badge>
                            ) : (
